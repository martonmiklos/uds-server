...
int write_can_frame(int can, struct canfd_frame *frame, int bytes)
{
    CAN_TxHeaderTypeDef header;
    header.DLC = frame->len;
    header.ExtId = frame->can_id;
    if (HAL_CAN_AddTxMessage(&hcan, &header, frame->data, NULL) != HAL_OK)
        return -1;
    return bytes;
}
...
void flow_control_push_to(int can, int id)
{
    ...
    while(gBufLengthRemaining > 0) {
        if(gBufLengthRemaining > 7) {
            ...
            nbytes = write_can_frame(can, &frame, CAN_MTU);
...
void isotp_send_to(int can, uint8_t *data, int size, int dest)
{
    struct canfd_frame frame;
    int left = size;
    int counter;
        ...
        if(no_flow_control) {
            while(left > 0) {
                if(left > 7) {
                    ...
                    write_can_frame(can, &frame, CAN_MTU);
                    counter++;
                    left -= 7;
                } else {
                    frame.len = left + 1;
                    ...
                    write_can_frame(can, &frame, frame.len);
                    left = 0;
                }
            }
        } else { // FC
...
void isotp_send(int can, uint8_t *data, int size)
{
...
void handle_vehicle_info(int can, struct canfd_frame frame)
{
    ...
    case 0x02: // Get VIN
        ...
        case 1:
            ...
            chksum = calc_vin_checksum((char*)buf, 17);
            buf[8] = chksum;
        ...
        case 3:  // At 3 the ISOTP spec gets flaky
            ...
            chksum = calc_vin_checksum((char*)buf, pktsize);
            buf[8] = chksum;
